...
import android.os.PowerManager;//added by sunjinbiao on 20120718 for bug[272]
import android.provider.Settings;//adde by sunjinbiao on 20121114 for disturbance mode
import android.media.RingtoneManager; //added by yuanhuawei
//Begin:added by sunjinbiao on 20120718 for bug[788]
import com.mediatek.audioprofile.AudioProfileService;
//import com.mediatek.audioprofile.AudioProfileManagerImpl;
//End:added by sunjinbiao on 20120718 for bug[788]
...
public class MessagingNotification {
    ...
    private static PowerManager.WakeLock  wl;//added by sunjinbiao on 20120718 for bug[272]
    public static final int NOTIFICATION_ID = 123;
    public static final int MESSAGE_FAILED_NOTIFICATION_ID = 789;
    public static final int DOWNLOAD_FAILED_NOTIFICATION_ID = 531;
	private final static String DISTURBANCE_MODE_ON="disturbanceMode";//adde by sunjinbiao on 20121114 for disturbance mode
    /**
    ...
    /* Begin: added by yuanhuawei 20131210 */
    private static final String NEW_UNREAD_SM_CONSTRAINT =
            "(" + Sms.TYPE + " = " + Sms.MESSAGE_TYPE_INBOX
            + " AND " + Sms.READ + " = 0)";
    /* End: added by yuanhuawei 20131210 */
    ...
    /* Begin: added by yuanhuawei 20131210 */
    private static final String NEW_UNREAD_MM_CONSTRAINT =
            "(" + Mms.MESSAGE_BOX + "=" + Mms.MESSAGE_BOX_INBOX
            + " AND " + Mms.READ + "=0"
            + " AND (" + Mms.MESSAGE_TYPE + "=" + MESSAGE_TYPE_NOTIFICATION_IND
            + " OR " + Mms.MESSAGE_TYPE + "=" + MESSAGE_TYPE_RETRIEVE_CONF + "))";
    ...
    public static void blockingUpdateNewMessageIndicator(Context context, long newMsgThreadId,
        ...
        int count = 0;
        count += addMmsNotificationInfos(context, threads);//mod by lzp
        count += addSmsNotificationInfos(context, threads);//mod by lzp
		...
		Intent i = new Intent("com.hskj.intent.ACTION_NOTIFICATION_LAUNCHER_COUNT");
		i.putExtra("className", "com.android.mms.ui.BootActivity");
	...
	/**Begin: added by lzp **/
    public static void notificationLauncherCount(Context context) {
		Set<Long> threads = new HashSet<Long>(4);
        int count = 0;
        count += addMmsNotificationInfos(context, threads);//mod by lzp
        count += addSmsNotificationInfos(context, threads);//mod by lzp
    ...
    private static final int addMmsNotificationInfos(//modified by lzp 
	   ...
	   /* Begin: added by yuanhuawei 20131210 */
           cursor.close();//added by xiashuaishuai on 20150130
	   cursor = SqliteWrapper.query(context, resolver, Mms.CONTENT_URI,
                            MMS_STATUS_PROJECTION, NEW_UNREAD_MM_CONSTRAINT,
                            null, Mms.DATE + " desc");
	    if (cursor != null){
	   	count = cursor.getCount();
	    }
    ...
    private static final int addSmsNotificationInfos(//modified by lzp
            Context context, Set<Long> threads) {
        ContentResolver resolver = context.getContentResolver();
	...
	cursor.close();
	   ...
	   /* Begin: added by yuanhuawei 20131210 */
	countcursor = SqliteWrapper.query(context, resolver, Sms.CONTENT_URI,
                            SMS_STATUS_PROJECTION, NEW_UNREAD_SM_CONSTRAINT,
                            null, Sms.DATE + " desc");
	  if (countcursor!= null){
...
//                | Intent.FLAG_ACTIVITY_CLEAR_TOP);
    ...
    private static void updateNotification(
        ...
        if(!pm.isScreenOn()&&Settings.System.getInt(context.getContentResolver(), DISTURBANCE_MODE_ON, -1)!=1){
	        lightUpTheScreen(pm);//added by sunjinbiao on 20120718 for bug[272]
		}
		/**End: added by lzp **/
    }
	/**Begin: added by lzp **/
	private static void lightUpTheScreen(PowerManager pm){
        if(wl ==null){
    ...
    private static void notifyFailed(Context context, boolean isDownload, long threadId,
		...
		/*Begin:added by liuzepeng*/
		PowerManager pm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
        if(!pm.isScreenOn()&&Settings.System.getInt(context.getContentResolver(), DISTURBANCE_MODE_ON, -1)!=1){
	        lightUpTheScreen(pm);
		}
		/*End:added by liuzepeng*/
    }
