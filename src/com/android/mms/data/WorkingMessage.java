...
import com.mediatek.common.featureoption.FeatureOption;/*Begin:added by sunjinbiao on 20130626 for bug[2595]*/
/**
...
/// @}
    ...
    public void removeAttachmentWhenOpened() {
 ...
 //add for attachment enhance
    ...
    public void addSelectedItems(String mPath) {
    	mSelectedItems.add(mPath);
    }
    
    public void removeSelectedItem(String mPath) {
    	mSelectedItems.remove(mPath);
    }
    
    public void removeSelectedSlidesAttachment(String mSelectedPath) {
        if (mSlideshow != null) {
            MmsLog.e(TAG, "mSlideshow.size() = " + mSlideshow.size());
            int size = mSlideshow.size();
            for (int i = 0; i < mSelectedItems.size(); i++) {
				if(mSelectedPath.equals(mSelectedItems.get(i))) {
					mSlideshow.remove(i);
				}
			}
            correctAttachmentState();
...
//            SlideshowEditor slideShowEditor = new SlideshowEditor(mActivity, mSlideshow);
	...
	if(FeatureOption.CENON_DB){
	    SharedPreferences sp= PreferenceManager.getDefaultSharedPreferences(mActivity);
	    boolean sign_enable = sp.getBoolean("sign_enable", true);
	    if(sign_enable){
	         String s =  sp.getString("sign_text", mActivity.getResources().getString(R.string.from_info));
	         msgText = msgText+"\n"+s;
		}
	}
    ...
    public void setHasMmsDraft(boolean hasMmsDraft) {
	    ...
	    //begin: added by wangyouyou
    public void delSelectedAttachment(int position) {
    	SlideshowEditor slideShowEditor = new SlideshowEditor(mActivity, mSlideshow);
    	slideShowEditor.removeSlide(position);
    	mSlideshow.remove(position);
    	updateState(HAS_ATTACHMENT, false, true);
    	slideShowEditor.setSlideshow(mSlideshow);
    }
    //end: added by wangyouyou
